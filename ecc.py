...
#     ASSUMPTION: b[i] for every i in range should be the same value
...
#     start_time = time.time()
    ...
    x_inverse_2 = mod_inverse(x**2, p)
################### try x_inverse_2 = x_inverse**2 %p ??? ########
#     print('x**2', x**2)
#     x_inverse_2 is always shorter
#     print('x**2', (x**2) % p)
#     print('x**-2', x_inverse_2)
# #     print('P Procedure:', P)
    P_prime = (x**2) % p * my_L + P + (x_inverse_2) * my_R
    ...
    for ii in range (original_length):
        ...
        for iii in range (log_length):
            if check_bit(ii, iii) == 1:
                product = (product * x_list[log_length - iii - 1])
            else:
